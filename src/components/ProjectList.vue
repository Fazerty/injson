...
<v-container>
  <v-card>
    ...
    <v-card-text style="height: 100px; position: relative">
      <v-btn absolute dark fab top small left color="grey" icon ripple @click.stop="appData.miniVariant = !appData.miniVariant">
        <v-icon v-html="appData.miniVariant? 'chevron_right':  'chevron_left'"></v-icon>
      </v-btn>
      <v-btn v-if="!appData.miniVariant" absolute dark fab top small right color="pink" icon ripple @click="createProject()">
        <v-icon>add</v-icon>
      </v-btn>
    </v-card-text>
  </v-card>
...
export default class ProjectList extends ExtVue {


  private created(this: ExtVue) {
